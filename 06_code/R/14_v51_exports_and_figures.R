...
class_out<-bind_rows(lapply(names(mods),function(nm){m<-mods[[nm]]; b<-coef(m); V<-vcov(m); do.call(rbind,lapply(c("2020","2023"),function(yr){p<-paste0("post",yr); it<-paste0("post",yr,":age75_2018"); mk<-function(label,w){e<-sum(w*b[names(w)]); s<-sqrt(as.numeric(t(w)%*%V[names(w),names(w),drop=FALSE]%*%w)); data.frame(outcome=labs[[nm]],wave=as.integer(yr),contrast=label,estimate=e,std_error=s,conf_low=e-1.96*s,conf_high=e+1.96*s,p_value=2*pnorm(-abs(e/s)),n_obs=nobs(m))}; bind_rows(mk("65-74 change",setNames(1,p)),mk("75+ change",setNames(c(1,1),c(p,it))),mk("75+ minus 65-74 change",setNames(1,it))) }))}))|>group_by(outcome)|>mutate(q_value=p.adjust(p_value,"BH"))|>ungroup()
