...
p4a <- p4a + scale_colour_manual(values=c("Higher-stable"=navy,"Intermediate"=gray,"Lower/declining"=coral)) + scale_fill_manual(values=c("Higher-stable"=navy,"Intermediate"=gray,"Lower/declining"=coral))
p4b <- ggplot(class_traj,aes(year,mean_score,group=class_label,colour=class_label)) + geom_ribbon(aes(ymin=conf_low,ymax=conf_high,fill=class_label),alpha=.12,colour=NA) + geom_line(linewidth=.7) + geom_point(size=1.5) + scale_colour_manual(values=c("Low-stable"=navy,"Moderate"=gray,"High/persistent"=coral)) + scale_fill_manual(values=c("Low-stable"=navy,"Moderate"=gray,"High/persistent"=coral)) + labs(x="Survey year",y="Mean 9-item depressive symptom score",title="CLASS depressive symptom trajectories") + theme_policy() + theme(legend.position="bottom")
